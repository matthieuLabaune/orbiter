<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold" style="color: var(--o-text);">Planning</h2>
    ...
    <div class="max-w-7xl mx-auto space-y-4" x-data="{ tab: 'kanban' }">
        ...
        <div class="flex items-center gap-1 border-b" style="border-color: var(--o-border);">
            <button @click="tab = 'kanban'" :class="tab === 'kanban' ? 'border-b-2' : 'opacity-60 hover:opacity-100'"
                    class="px-4 py-2 text-sm font-medium transition-colors cursor-pointer flex items-center gap-2" :style="tab === 'kanban' ? 'color: var(--o-text); border-color: var(--o-accent);' : 'color: var(--o-text-4);'">
                <x-lucide-columns-3 class="w-4 h-4" />
                Kanban
            </button>
            <button @click="tab = 'gantt'" :class="tab === 'gantt' ? 'border-b-2' : 'opacity-60 hover:opacity-100'"
                    class="px-4 py-2 text-sm font-medium transition-colors cursor-pointer flex items-center gap-2" :style="tab === 'gantt' ? 'color: var(--o-text); border-color: var(--o-accent);' : 'color: var(--o-text-4);'">
                <x-lucide-gantt-chart class="w-4 h-4" />
                Gantt
            </button>
            <button @click="tab = 'list'" :class="tab === 'list' ? 'border-b-2' : 'opacity-60 hover:opacity-100'"
                    class="px-4 py-2 text-sm font-medium transition-colors cursor-pointer flex items-center gap-2" :style="tab === 'list' ? 'color: var(--o-text); border-color: var(--o-accent);' : 'color: var(--o-text-4);'">
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y" style="border-color: var(--o-border);">
                        @foreach($tasks as $task)
                            ...
                            @endphp
                            <tr class="transition-colors" style="--tw-divide-color: var(--o-border);">
                                <td class="px-4 py-3">
                                    <a href="{{ route('projects.tasks.show', [$project, $task]) }}" style="color: var(--o-text);">{{ $task->title }}</a>
                                </td>
                                <td class="px-4 py-3 text-xs" style="color: var(--o-text-4);">{{ $task->module?->name ?? '—' }}</td>
                                <td class="px-4 py-3 text-xs" style="color: var(--o-text-4);">{{ $task->assignee?->name ?? '—' }}</td>
                                ...
                                <td class="px-4 py-3 text-xs whitespace-nowrap" style="color: var(--o-text-4);">
