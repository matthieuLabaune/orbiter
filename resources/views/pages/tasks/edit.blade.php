<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('projects.tasks.show', [$project, $task]) }}" class="transition-colors" style="color: var(--orbiter-text-muted);">
                <x-lucide-arrow-left class="w-5 h-5" />
            </a>
            <h2 class="text-xl font-semibold" style="color: var(--orbiter-text);">Modifier la tâche</h2>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        <form action="{{ route('projects.tasks.update', [$project, $task]) }}" method="POST"
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label for="assignee_id" class="block text-sm font-medium mb-1" style="color: var(--orbiter-text-secondary);">Assigné à</label>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    <label for="progress" class="block text-sm font-medium mb-1" style="color: var(--orbiter-text-secondary);">Progression (%)</label>
                    <input type="number" name="progress" id="progress" value="{{ old('progress', $task->progress) }}" min="0" max="100"
                           class="input-field">
                </div>
                <div>
                    <label for="start_date" class="block text-sm font-medium mb-1" style="color: var(--orbiter-text-secondary);">Début</label>
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $task->start_date?->format('Y-m-d')) }}"
                           class="input-field">
                </div>
                <div>
                    <label for="end_date" class="block text-sm font-medium mb-1" style="color: var(--orbiter-text-secondary);">Fin</label>
            ...
            @if($allTasks->isNotEmpty())
                ...
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--orbiter-text-secondary);">Bloqué par</label>
                    <div class="max-h-32 overflow-y-auto space-y-1.5 rounded-lg p-3" style="border: 1px solid var(--orbiter-border);">
                        @foreach($allTasks as $t)
                            <label class="flex items-center gap-2 text-sm cursor-pointer">
                                ...
                                <span style="color: var(--orbiter-text-secondary);">{{ $t->title }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('projects.tasks.show', [$project, $task]) }}" class="px-4 py-2 text-sm transition-colors" style="color: var(--orbiter-text-muted);">Annuler</a>
