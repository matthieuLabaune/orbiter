<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold" style="color: var(--orbiter-text);">Deploy Readiness</h2>
    </x-slot>
    ...
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="surface p-6">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold" style="color: var(--orbiter-text);">Statut actuel</h3>
                ...
                @foreach($currentReadiness['modules'] as $name => $status)
                    ...
                    <div class="flex items-center justify-between p-3 rounded-lg border {{ $modGo ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-red-500/20 bg-red-500/5' }}">
                        <div>
                            ...
                            <div class="text-xs" style="color: var(--orbiter-text-muted);">{{ $status['reason'] }}</div>
                        </div>
            ...
            @if(!empty($currentReadiness['blocking_items']))
                <div class="mt-4 p-3 bg-red-500/5 border border-red-500/20 rounded-lg">
                    ...
                    @foreach($currentReadiness['blocking_items'] as $item)
                        <div class="text-xs py-1" style="color: var(--orbiter-text-muted);">
        ...
        @if($reviews->isNotEmpty())
            <div class="surface p-6">
                <h3 class="text-sm font-medium uppercase tracking-wider mb-4" style="color: var(--orbiter-text-muted);">Historique</h3>
                <div class="space-y-3">
                    @foreach($reviews as $review)
                        <div class="flex items-center justify-between py-2 border-b last:border-0" style="border-color: var(--orbiter-border);">
                            <div>
                                <span class="font-mono text-sm" style="color: var(--orbiter-text);">{{ $review->ref }}</span>
                                <span class="text-sm" style="color: var(--orbiter-text-muted);">→ {{ $review->target_version }}</span>
                            </div>
                            <div class="flex items-center gap-3">
                                <x-ui.status-dot :status="$review->result === 'go' ? 'go' : 'no_go'" />
                                <span class="text-xs" style="color: var(--orbiter-text-muted);">{{ $review->decided_at?->format('d/m/Y') }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
