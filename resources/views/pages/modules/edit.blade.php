<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto">
        <form action="{{ route('projects.modules.update', [$project, $module]) }}" method="POST"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full bg-gray-50 dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-lg px-3 py-2 text-gray-900 dark:text-white placeholder-slate-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">{{ old('description', $module->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            @if($allModules->isNotEmpty())
                <div>
                    <label class="block text-sm font-medium text-gray-600 dark:text-slate-300 mb-2">Dépendances</label>
                    <div class="space-y-2 bg-gray-50 dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-lg p-3">
                        @foreach($allModules as $otherModule)
                            <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-slate-300 cursor-pointer hover:text-gray-900 dark:hover:text-white transition-colors">
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('projects.modules.show', [$project, $module]) }}" class="px-4 py-2 text-sm text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                    Annuler
                </a>
                <button type="submit"
        ...
        <div class="mt-8 bg-red-500/5 border border-red-500/20 rounded-xl p-6">
            ...
            <p class="text-sm text-gray-400 dark:text-slate-500 mb-4">Supprimer ce module et toutes ses données de manière irréversible.</p>
