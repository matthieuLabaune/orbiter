<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold" style="color: var(--orbiter-text);">Architecture Decision Records</h2>
    ...
    <div class="max-w-4xl mx-auto space-y-4">
                ...
                <div class="flex items-start justify-between">
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <span class="font-mono text-sm" style="color: var(--orbiter-accent);">{{ $adr->ref }}</span>
                            <x-ui.badge :color="$statusColors[$adr->status] ?? 'slate'">{{ $statusLabels[$adr->status] ?? $adr->status }}</x-ui.badge>
                        </div>
                        <h3 class="font-medium group-hover:text-blue-400 transition-colors" style="color: var(--orbiter-text);">{{ $adr->title }}</h3>
                        @if($adr->modules->isNotEmpty())
                            <div class="flex flex-wrap gap-1 mt-2">
                                @foreach($adr->modules as $mod)
                                    <span class="text-xs px-1.5 py-0.5 rounded" style="background: var(--orbiter-surface-2); color: var(--orbiter-text-muted);">{{ $mod->name }}</span>
                                @endforeach
                            </div>
                        @endif
                    </div>
                    <div class="text-xs" style="color: var(--orbiter-text-muted);">
        ...
        @empty
            <div class="text-center py-16">
                ...
                <h3 class="text-lg font-medium mb-2" style="color: var(--orbiter-text-secondary);">Aucun ADR</h3>
                <p class="mb-6" style="color: var(--orbiter-text-muted);">Documentez votre première décision d'architecture.</p>
            </div>
        @endforelse
    </div>
</x-app-layout>
