    ...
    <div class="max-w-4xl mx-auto space-y-4">
                ...
                <div class="flex items-start justify-between">
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <span class="font-mono text-blue-400 text-sm">{{ $adr->ref }}</span>
                            <x-ui.badge :color="$statusColors[$adr->status] ?? 'slate'">{{ $statusLabels[$adr->status] ?? $adr->status }}</x-ui.badge>
                        </div>
                        <h3 class="text-white font-medium group-hover:text-blue-400 transition-colors">{{ $adr->title }}</h3>
                        @if($adr->modules->isNotEmpty())
                            <div class="flex flex-wrap gap-1 mt-2">
                                @foreach($adr->modules as $mod)
                                    <span class="text-xs px-1.5 py-0.5 bg-slate-800 text-slate-400 rounded">{{ $mod->name }}</span>
                                @endforeach
                            </div>
                        @endif
                    </div>
        ...
        @empty
            <div class="text-center py-16">
                <x-lucide-file-text class="w-12 h-12 text-slate-600 mx-auto mb-4" />
                <h3 class="text-lg font-medium text-slate-300 mb-2">Aucun ADR</h3>
                <p class="text-slate-500 mb-6">Documentez votre première décision d'architecture.</p>
            </div>
        @endforelse
    </div>
</x-app-layout>
