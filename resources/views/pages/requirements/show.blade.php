            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <span class="font-mono text-blue-600 dark:text-blue-400 text-lg">{{ $requirement->ref }}</span>
    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                @if($requirement->description)
                    <div class="bg-white dark:bg-slate-900/80 border border-gray-200 dark:border-slate-700/50 rounded-xl p-5">
                        ...
                        <div class="text-gray-600 dark:text-slate-300 whitespace-pre-wrap">{{ $requirement->description }}</div>
                    </div>
                @endif
                ...
                @if($requirement->acceptance_criteria)
                    <div class="bg-white dark:bg-slate-900/80 border border-gray-200 dark:border-slate-700/50 rounded-xl p-5">
                        <h3 class="text-sm font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-3">Critères d'acceptation</h3>
                        <div class="text-gray-600 dark:text-slate-300 whitespace-pre-wrap font-mono text-sm">{{ $requirement->acceptance_criteria }}</div>
                    </div>
                @endif

                {{-- Tests --}}
                <div class="bg-white dark:bg-slate-900/80 border border-gray-200 dark:border-slate-700/50 rounded-xl p-5">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-3">Tests liés</h3>
                    @if($requirement->tests->isEmpty())
                        <p class="text-gray-400 dark:text-slate-500 text-sm">Aucun test lié à cette exigence.</p>
                    @else
                        <table class="w-full text-sm">
                            <thead class="text-xs text-gray-400 dark:text-slate-500 uppercase">
                                <tr>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-slate-700/50">
                                @foreach($requirement->tests as $test)
                                    ...
                                    @endphp
                                    <tr>
                                        <td class="py-2 font-mono text-blue-600 dark:text-blue-400">{{ $test->ref }}</td>
                                        <td class="py-2 text-gray-600 dark:text-slate-300">{{ $test->title }}</td>
                                        <td class="py-2">
                    ...
                    <div class="bg-white dark:bg-slate-900/80 border border-gray-200 dark:border-slate-700/50 rounded-xl p-5">
                        ...
                        <div class="space-y-3">
                            @foreach($requirement->versions->sortByDesc('version') as $version)
                                <div class="border-l-2 border-gray-200 dark:border-slate-700 pl-4">
                                    ...
                                    @if($version->change_reason)
                                        <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">{{ $version->change_reason }}</p>
                                    @endif
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="space-y-6">
                {{-- Risk Score --}}
                <div class="bg-white dark:bg-slate-900/80 border border-gray-200 dark:border-slate-700/50 rounded-xl p-5">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-3">Score de risque</h3>
                    <div class="text-center mb-4">
                        <x-project.risk-score :score="$requirement->risk_score" class="text-lg" />
                    </div>
                    <div class="grid grid-cols-3 gap-2 text-center text-xs">
                        <div>
                            <div class="text-gray-400 dark:text-slate-500">Impact</div>
                            <div class="text-gray-900 dark:text-white font-mono">{{ $requirement->risk_impact ?? '—' }}</div>
                        </div>
                        <div>
                            <div class="text-gray-400 dark:text-slate-500">Probabilité</div>
                            <div class="text-gray-900 dark:text-white font-mono">{{ $requirement->risk_probability ?? '—' }}</div>
                        </div>
                        <div>
                            <div class="text-gray-400 dark:text-slate-500">Détectabilité</div>
                            <div class="text-gray-900 dark:text-white font-mono">{{ $requirement->risk_detectability ?? '—' }}</div>
                        </div>
                    </div>
                </div>
                ...
                {{-- Tasks --}}
                <div class="bg-white dark:bg-slate-900/80 border border-gray-200 dark:border-slate-700/50 rounded-xl p-5">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-3">Tâches ({{ $requirement->tasks->count() }})</h3>
                    @forelse($requirement->tasks as $task)
                        <div class="flex items-center justify-between py-1.5 text-sm">
                            <span class="text-gray-600 dark:text-slate-300 truncate">{{ $task->title }}</span>
                            @php
                    ...
                    @empty
                        <p class="text-xs text-gray-400 dark:text-slate-500">Aucune tâche liée.</p>
                    @endforelse
                </div>
                ...
                @if($requirement->lessons->isNotEmpty())
                    <div class="bg-white dark:bg-slate-900/80 border border-gray-200 dark:border-slate-700/50 rounded-xl p-5">
                        <h3 class="text-sm font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-3">Lessons Learned</h3>
                        @foreach($requirement->lessons as $lesson)
                            <div class="py-2 border-b border-gray-100 dark:border-slate-700/50 last:border-0">
                                <div class="text-sm text-gray-600 dark:text-slate-300">{{ $lesson->title }}</div>
                                <div class="text-xs text-gray-400 dark:text-slate-500 font-mono">{{ $lesson->ref }}</div>
                            </div>
                        @endforeach
                    </div>
                @endif
                ...
                @if($requirement->anomalies->isNotEmpty())
                    <div class="bg-white dark:bg-slate-900/80 border border-gray-200 dark:border-slate-700/50 rounded-xl p-5">
                        <h3 class="text-sm font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider mb-3">Anomalies</h3>
                        @foreach($requirement->anomalies as $anomaly)
                            <div class="py-2 border-b border-gray-100 dark:border-slate-700/50 last:border-0">
                                <div class="flex items-center gap-2">
                                    <span class="text-sm font-mono text-red-400">{{ $anomaly->ref }}</span>
                                    <span class="text-sm text-gray-600 dark:text-slate-300">{{ $anomaly->title }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
