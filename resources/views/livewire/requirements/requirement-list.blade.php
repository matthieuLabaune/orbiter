<div>
    {{-- Filters --}}
    <div class="flex flex-wrap items-center gap-3 bg-slate-900/80 border border-slate-700/50 rounded-xl p-4 mb-4">
        <div class="relative">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Rechercher..."
                   class="bg-slate-800 border border-slate-600 rounded-lg pl-9 pr-3 py-1.5 text-sm text-white placeholder-slate-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 w-52">
        </div>
        ...
        @if($search || $moduleFilter || $statusFilter || $priorityFilter)
            <button wire:click="$set('search', ''); $set('moduleFilter', ''); $set('statusFilter', ''); $set('priorityFilter', '')"
                    class="text-xs text-slate-500 hover:text-slate-300 cursor-pointer">
                Effacer les filtres
            </button>
        @endif
        ...
        <table class="w-full text-sm text-left">
            ...
            <tbody class="divide-y divide-slate-700/50">
                @forelse($requirements as $req)
                    <tr class="hover:bg-slate-800/30 transition-colors" wire:key="req-{{ $req->id }}">
                        <td class="px-4 py-3">
                            <a href="{{ route('projects.requirements.show', [$project, $req]) }}"
                               class="font-mono text-blue-400 hover:text-blue-300">{{ $req->ref }}</a>
                        </td>
                        ...
                        <td class="px-4 py-3 text-xs text-slate-400">{{ $req->module?->name }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-xs text-slate-400">{{ $req->tests->count() }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-slate-500">Aucune exigence trouvée.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
