    ...
    @else
        <div class="space-y-2 max-h-80 overflow-y-auto">
            @foreach($alerts as $alert)
                ...
                @endphp
                <a href="{{ $alert['url'] }}"
                   class="flex items-start gap-3 p-3 rounded-lg border-l-2 {{ $borderColors[$alert['type']] ?? 'border-l-slate-500' }} bg-slate-800/50 hover:bg-slate-800 transition-colors">
                    <x-dynamic-component :component="'lucide-' . $alert['icon']" class="w-4 h-4 mt-0.5 shrink-0 {{ $iconColors[$alert['type']] ?? 'text-slate-400' }}" />
                    <div class="min-w-0">
                        <div class="text-sm text-slate-200">{{ $alert['message'] }}</div>
                        @if($alert['detail'])
                            <div class="text-xs text-slate-500 truncate">{{ $alert['detail'] }}</div>
                        @endif
                        @if($alert['module'])
                            <span class="inline-block mt-1 text-[10px] px-1.5 py-0.5 bg-slate-700 text-slate-400 rounded">{{ $alert['module'] }}</span>
                        @endif
                    </div>
                </a>
            @endforeach
